  ...
  if(!registered){
    ...
    octokit.hook.after('request', async (result, options)=>{
      if(result.status ===200 && result.headers.etag){
        ...
        dispatch('cache/addRequest', {uri: options.url, etag: result.headers.etag, filePath: 'testCache.json'})
      }
    })
...
export async function getAvailableModuleFolders(state, dispatch){
